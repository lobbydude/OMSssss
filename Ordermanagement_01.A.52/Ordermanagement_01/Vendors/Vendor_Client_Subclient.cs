...
namespace Ordermanagement_01.Vendors
{
    public partial class Vendor_Client_Subclient : Form
    {
        ...
        public Vendor_Client_Subclient(int Userid)
        {
            InitializeComponent();
            userid = Userid;
        ...
        private void txt_Search_Vendor_OrderType_TextChanged(object sender, EventArgs e)
       {
        ...
        private void txt_Search_Vendor_stcounty_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                DataView dtsearch = new DataView(dtsear);
                dtsearch.RowFilter = "Vendor_Name like '%" + txt_Search_Vendor_stcounty.Text.ToString() + "%' or Client_Name like '%" + txt_Search_Vendor_stcounty.Text.ToString() + "%' or Sub_ProcessName like '%" + txt_Search_Vendor_stcounty.Text.ToString() + "%'";
                //DataTable temp = new DataTable();
                temp = dtsearch.ToTable();
                if (temp.Rows.Count > 0)
                {
                    ...
                    for (int ven = 0; ven < temp.Rows.Count; ven++)
                    {
                        ...
                        grd_Search_Client_sub.Rows[ven].Cells[2].Value = temp.Rows[ven]["Client_Name"].ToString();
                        grd_Search_Client_sub.Rows[ven].Cells[3].Value = temp.Rows[ven]["Sub_ProcessName"].ToString();
