...
namespace Ordermanagement_01
{
    public partial class Rework_Search_Order : Form
    {
        ...
        string User_Role;
        public Rework_Search_Order(int USER_ID,string USER_ROLE)
        {
            InitializeComponent();
            userid = USER_ID;
            User_Role = USER_ROLE;
        ...
        private void grd_order_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 0)
            {
                Ordermanagement_01.Rework_Superqc_Order_Entry orderentry = new Ordermanagement_01.Rework_Superqc_Order_Entry(int.Parse(grd_order.Rows[e.RowIndex].Cells[13].Value.ToString()), userid, "Rework",User_Role);
                orderentry.Show();
            }
        }
