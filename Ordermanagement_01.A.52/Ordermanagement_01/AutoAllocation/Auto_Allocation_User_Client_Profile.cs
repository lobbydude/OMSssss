...
namespace Ordermanagement_01.AutoAllocation
{
    public partial class Auto_Allocation_User_Client_Profile : Form
    {
        ...
        int Auto_User_Id;
        bool Client_Status, Search_Status, Search_Qc_Status, Typing_Status, Typing_Qc_Status, Upload_Status,Client_Priority;
        DialogResult dialogResult;
        public Auto_Allocation_User_Client_Profile()
        {
            InitializeComponent();
        }
        ...
        private void Bind_Clients_List()
        {
            ...
            if (dtclient.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtclient.Rows.Count; i++)
                {
                    ...
                    Grid_Client_Task.Rows[i].Cells[7].Value = dtclient.Rows[i]["Client_Id"].ToString();
                ...
                for (int i = 0; i < Grd_User_List.Rows.Count; i++)
                {
                    ...
                    if (Check == true)
                    {
                        ...
                        for (int j = 0; j < Grid_Client_Task.Rows.Count; j++)
                        {
                            ...
                            int Client_Id = int.Parse(Grid_Client_Task.Rows[j].Cells[7].Value.ToString());
                            ...
                            if (Grid_Client_Task.Rows[j].Cells[8].Value != null && Grid_Client_Task.Rows[j].Cells[8].Value.ToString() == "True") { Client_Priority = true; } else { Client_Priority = false; }
                    ...
                    if (dtgetlist.Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dtgetlist.Rows.Count; i++)
                        {
                            ...
                            for (int j = 0; j < Grid_Client_Task.Rows.Count; j++)
                            {
                                    ...
                                    if (Grid_Client_Task.Rows[j].Cells[7].Value.ToString() == dtgetlist.Rows[i]["Client_Id"].ToString())
                                    {
        ...
        private void Grid_Client_Task_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex != -1)
            {
                if (e.ColumnIndex == 8)
                {
                    bool ischeck = (bool)Grid_Client_Task[8, e.RowIndex].FormattedValue;
                    if (ischeck == false)
                    {
