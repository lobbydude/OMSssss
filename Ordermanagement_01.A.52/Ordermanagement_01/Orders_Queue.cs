...
namespace Ordermanagement_01
{
    ...
    public partial class Orders_Queue : Form
    {
        ...
        public Orders_Queue(int User_ID,string User_Role_id)
        {
            InitializeComponent();
            userid = User_ID;
...
BindingFlags.Instance | BindingFlags.SetProperty, null,grd_order, new object[] { true });
        ...
        private void grd_order_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.ColumnIndex == 1)
            {
                ...
                Ordermanagement_01.Order_Entry OrderEntry = new Ordermanagement_01.Order_Entry(int.Parse(grd_order.Rows[e.RowIndex].Cells[19].Value.ToString()), userid, userroleid);
                OrderEntry.Show();
            }
            if (e.ColumnIndex == 20)
            {
