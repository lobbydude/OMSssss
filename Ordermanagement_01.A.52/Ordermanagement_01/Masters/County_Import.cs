...
{
    public partial class County_Import : Form
    {
        ...
        string Client_Name;
        public County_Import(int USER_ID)
        {
            InitializeComponent();
                    ...
                    try
                    {
                        ...
                        for (int i = 0; i < data.Rows.Count; i++)
                        {
                            if (data.Rows[i]["State"].ToString() != "" || data.Rows[i]["County"].ToString() != "" || data.Rows[i]["State"].ToString() != null || data.Rows[i]["County"].ToString() != null && data.Rows[i]["CountyType"].ToString() != "" && data.Rows[i]["CountyType"].ToString() != null)
                            {
                                ...
                                dtcountyType = dataaccess.ExecuteSP("Sp_County", htcountyType);
                                if (dtcounty.Rows.Count != 0)
                                {
                            ...
                            else
                            {
                                grd_Client_cost.Rows.Clear();
                                string title = "Empty!";
                                MessageBox.Show("Check Empty Cells in Excel",title);
                            }
            ...
            //cProbar.startProgress();
             ...
             for (int i = 0; i < grd_Client_cost.Rows.Count; i++)
             {
                 ...
                 if (error != "Error")
                 {
                         ...
                         System.Data.DataTable dtcheck = new System.Data.DataTable();
                         htcheck.Add("@Trans", "CHECK_COUNTY_BY_NAME");
                         htcheck.Add("@State_Id", State);
                         htcheck.Add("@County", County_Name);
                         dtcheck = dataaccess.ExecuteSP("Sp_County", htcheck);
                         if (dtcheck.Rows.Count > 0)
                         {
                         ...
                         if (Check_value == 0)
                         {
                             ...
                             htabsinsert.Add("@Trans", "COUNTY_INSERT");
                             htabsinsert.Add("@State_Id", State);
                             htabsinsert.Add("@County", County_Name);
                             htabsinsert.Add("@Type", County_Type);
                             dtabsinsert = dataaccess.ExecuteSP("Sp_County", htabsinsert);
                         ...
                         else if (Check_value > 0)
                         {
                             Hashtable htabsinsert = new Hashtable();
                             System.Data.DataTable dtabsinsert = new System.Data.DataTable();
                             htabsinsert.Add("@Trans", "COUNTY_UPDATE");
                             htabsinsert.Add("@State_Id", State);
                             htabsinsert.Add("@County", County_Name);
                             htabsinsert.Add("@Type", County_Type);
                             dtabsinsert = dataaccess.ExecuteSP("Sp_County", htabsinsert);
