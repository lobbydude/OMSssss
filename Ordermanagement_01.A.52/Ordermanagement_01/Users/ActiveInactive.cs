            ...
            if (e.RowIndex!=-1)
            {
                if (grd_ActInact.Rows[e.RowIndex].Cells[5].Value.ToString() != null)
                {
                    ...
                    if (e.ColumnIndex == 4)
                    {
                        ...
                        {
                            var op = MessageBox.Show("Do You Want to Deactivate the User", "confirmation", MessageBoxButtons.YesNo);
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            ...
            else if(ddl_UserState.SelectedIndex>0)
            {
                if (currentpageindex + 1 <= (dtactive.Rows.Count / pagesize))
                {
                    ...
                    this.Cursor = Cursors.WaitCursor;
                    currentpageindex++;
                    if (currentpageindex == (int)Math.Ceiling(Convert.ToDecimal(dtactive.Rows.Count) / pagesize) - 1)
                    {
                        btnNext.Enabled = false;
                        btnLast.Enabled = false;
                    }
